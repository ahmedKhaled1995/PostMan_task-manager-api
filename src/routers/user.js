...
router.post("/users/logout", auth, async (req, res) => {
    try{
        req.authorizedUser.tokens = req.authorizedUser.tokens.filter((tokenObj)=>{    // note that tokenObj is an object with a token key whose value is a string
...
const uplaoadAvatars = multer({
    ...
    fileFilter: (req, file, callback) => {
        ...
        const isValidUpload = (fileName.endsWith(".jpg") || fileName.endsWith(".jpeg") || fileName.endsWith(".png"));
        if(isValidUpload){
...
router.post("/users/me/avatar", auth, uplaoadAvatars.single("avatar"), async(req, res) => {
...
router.delete("/users/me/avatar", auth, uplaoadAvatars.single("avatar"), async (req, res) =>{
